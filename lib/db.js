...
export async function listRecords() {
  ...
  return sql`
    SELECT r.id, r.code, r.created_at, r.updated_at,
      lr.revision_label, lr.status, lr.product_name, lr.customer,
      EXISTS (SELECT 1 FROM ppwr_revisions p WHERE p.record_id=r.id AND p.status='published') AS has_published
    FROM ppwr_records r
    LEFT JOIN LATERAL (
      SELECT revision_label, status, product_name, customer
...
export async function getPublishedRecordByCode(code) {
  ...
  const rows = await sql`
    ...
    JOIN LATERAL (
      ...
      WHERE x.record_id = r.id AND x.status = 'published'
      ORDER BY x.published_at DESC NULLS LAST, x.created_at DESC, x.id DESC
      LIMIT 1
    ) v ON true
